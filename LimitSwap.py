...
parser.add_argument("-c", "--cooldown", type=int, help="Slow the bot down to COOLDOWN queries per second")
...
def load_tokens_file(tokens_path, load_message=True):
    ...
    # _STABLE_BASES          - the typical stable symbols used on this exchange
    ...
    for token in tokens:
        ...
        # Make sure addresses are formatted properly
        ...
        if token['USECUSTOMBASEPAIR'] == 'false':
            token['BASESYMBOL'] = settings['_EXCHANGE_BASE_SYMBOL']
            token['BASEADDRESS'] = settings['_EXCHANGE_BASE_ADDRESS']
...
def token_list_report(tokens, all_pairs=False):
    ...
    #
    # returns: an array of all SYMBOLS we reported
    ...
    for token in tokens:
        if all_pairs == True or token["ENABLED"] == 'true':
            ...
            token_list_array.append(token['_PAIR_SYMBOL'])
            token_list = token_list + token['_PAIR_SYMBOL']

    if len(token_list_array) == 0:
        printt("No tokens are enabled for trading.")
    else:
        printt("Quantity of tokens attempting to trade:", len(token_list_array), "(" , token_list , ")")
...
if settings['EXCHANGE'] == 'pancakeswap':
    ...
    settings['_EXCHANGE_BASE_ADDRESS'] = Web3.toChecksumAddress("0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c")
...
def build_extended_base_configuration (token_dict):
    ...
    for stable_token in settings['_STABLE_BASES']:
        ...
        new_token.update({
                        ...
                        "BASEADDRESS": Web3.toChecksumAddress(settings['_STABLE_BASES'][stable_token]['address']),
...
def check_pool(inToken, outToken, symbol):
    ...
    printt_debug("EXITING: check_pool()")
    return pooled
...
def check_liquidity(token_dict, all_tokens_dict) :
    # Function: check_liquidity()
    # ----------------------------
    # check to see if liquidity is available for this pair and check to see if any other pairs should be disabled due to finding
    # liquidity on this pair
    #
    # token_dict - the current token we are trying to trade
    # all_tokens_dict - the dictionary containing all tokens the user wants to trade
    #
    # returns: true - if the token is ready for trading
    #          false - if the token is not ready for trading
    #
...
def run():
    ...
    try:
        ...
        number_of_enabled_tokens = len(token_list_report(tokens))
        ...
        loopcheck_checkfrequency = approximate_queries_per_second

        # Used to track the number of times we've gone through the for loop with an enabled token
        #  that way if any tokens have changed, we can report on it.
        enabled_token_for_loop_increment = number_of_enabled_tokens
        ...
        while True:
            ...
            # Check to see if the number of enabled tokens has changed
            if enabled_token_for_loop_increment != number_of_enabled_tokens:
                printt_info("The number of trading pairs has changed")
                number_of_enabled_tokens = len(token_list_report(tokens))

            
            enabled_token_for_loop_increment = 0
            for token in tokens:
                
                # Don't time our loops unless we're dealing with an enabled token
                if token['ENABLED'] == 'true':
                    if loopcheck_nextcheck > 0:
                        ...
                        # If the bot is doing more than a predefined number of queries a second out of sonic mode, slow it down (provide about a 10% buffer)
                        if loop_time  < (1 / (approximate_queries_per_second * 1.10)) :
                            printt_debug(loop_time)
                            printt_info ("Bot is moving too fast. Slowing down to approximately", approximate_queries_per_second , "queries per second.")
                            bot_too_fast_cooldown = (1 / approximate_queries_per_second) - loop_time



                if token['ENABLED'] == 'true':        
                    
                    enabled_token_for_loop_increment += 1
                    ...
                    if token['_LIQUIDITY_READY'] == False:
                        ...
                        if check_liquidity(token, tokens) == False:
                            continue
                        
                        elif first_liquidity_check == True and command_line_args.reject_instant_liquidity:
